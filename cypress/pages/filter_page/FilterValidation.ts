...
validateCardFilters(filters:Record<string,any>,cardIndex: number=0) {
    ...
    cy.get(PROPERTY_CARD.selector!).filter(':visible').eq(cardIndex).within(()=>{
        for (const filterName in filters) {
            ...
            logger.validation(`Checking filter ${filterName}`);
            switch(filterName){
                ...
                break;
                ///////////////////////////////////////////////////////////////////// 
                default:logger.error(`No assertion defined for ${filterName}`);
            }
        }
        cy.then(()=>{
            if (validationErrors.length>0){
                ...
                logger.success("All filter validations passed successfully");
            }
});
});
}
...
assertBedroomsBathrooms(value:{bedrooms?:number;bathrooms?:number},validationErrors:string[],filterName: string){
    cy.get(PROPERTY_CARD_UNIT_CONFIGURATION.selector!,{timeout:3000}).invoke('text').then(text=>{
        ...
        logger.validation(`Bedrooms: ${bedrooms} | Bathrooms: ${bathrooms}`);
        if(value.bedrooms!==undefined){
            if(bedrooms<value.bedrooms){
                ...
                logger.success(`${filterName}: Bedrooms valid | Actual: ${bedrooms} | Expected: ${value.bedrooms}`);
            }
        }
        if(value.bathrooms!==undefined){
            if(bathrooms<value.bathrooms){
                ...
                logger.success(`${filterName}: Bathrooms valid | Actual: ${bathrooms} | Expected: ${value.bathrooms}`);
            }
        }
    });
}
...
assertReviewScore(value:string,validationErrors:string[],filterName:string){
    cy.get(REVIEW_SCORE.selector!).invoke('text').then(text=>{
            ...
            logger.validation(`Expected score: ${minScore}`);
            logger.validation(`Actual score: ${score}`);
            if(score<minScore){
                validationErrors.push(`${filterName}: Review score is ${score} and expected is ${minScore}`)
            }
            else{
                logger.success(`${filterName}: Review Score is valid: ${score}`);
            }
        });
}
/**
